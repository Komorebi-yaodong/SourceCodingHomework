...
def huffman_zip(file_name, n):
    ...
    max_len = 0
    for it in code_table:
        it_len = len(code_table[it]) // 8
        if len(it) % 8 != 0:
            it_len += 1
        max_len = max(it_len, max_len)
    ...
    while True:
        ...
        b = c.to_bytes(max_len, byteorder='big')
        f_out.write(b)
    f_out.close()
...
def huffman_unzip(zip_name):
    with open(zip_name, 'rb') as f:
        ...
        # 找到长度
        max_len = int(str(f.readline(), encoding="utf-8").strip())
        # 找到名字
