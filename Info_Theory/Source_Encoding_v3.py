...
def huffman_zip(file_name, n):
    ...
    for k, v in code_table.items():
        ...
        print("k:", decode_table[v], type(decode_table[v]))
    ...
    # 图形展示设置
    with open(file_name, 'rb') as f_pre:
        total_bar = len(f_pre.read())
    ...
    while True:
        ...
        f_out.write(b)
        pb_number = (pb * 100) // (total_bar // 8)
        if pb_number != len_pb:
            print("process:".ljust(pb_number + 8, '='), end='>')
            print("".ljust(100 - pb_number, ' '), end='||\n')
            len_pb = pb_number
    f_in.close()
    f_out.close()
